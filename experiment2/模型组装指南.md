# Experiment2 模型组装指南

## 📋 现状

### ✅ 已实现的模块 (11个)

**Stage1: 编码器** (2个)
- ✅ `stage1_encoder/clip_text_encoder.py` - CLIP文本编码器
- ✅ `stage1_encoder/global_context_extractor.py` - 全局上下文提取器

**Stage2: 解码器** (3个)
- ✅ `stage2_decoder/query_initializer.py` - 查询初始化器
- ✅ `stage2_decoder/context_gating.py` - 上下文门控
- ✅ `stage2_decoder/text_conditioner.py` - 文本调节器

**Stage3: 预测** (2个)
- ✅ `stage3_prediction/classification_head.py` - 分类头
- ✅ `stage3_prediction/regression_head.py` - 回归头

**Stage4: 监督** (3个)
- ✅ `stage4_supervision/box_loss.py` - 边界框损失
- ✅ `stage4_supervision/global_contrast_loss.py` - 全局对比损失
- ✅ `stage4_supervision/matcher.py` - 匈牙利匹配器

**推理** (1个)
- ✅ `inference/post_processor.py` - 后处理器 (NMS)

**新增** (3个)
- ✅ `utils/dataloader.py` - 数据加载器
- ✅ `utils/evaluation.py` - mAP评估
- ✅ `utils/box_utils.py` - 边界框工具

---

## 🏗️ 需要组装的完整模型

### 文件: `models/context_guided_detector.py`

```python
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
上下文引导检测器 - 完整模型

整合所有模块
"""

import torch
import torch.nn as nn

from stage1_encoder.clip_text_encoder import CLIPTextEncoder
from stage1_encoder.global_context_extractor import GlobalContextExtractor
from stage2_decoder.query_initializer import QueryInitializer
from stage2_decoder.context_gating import ContextGating
from stage2_decoder.text_conditioner import TextConditioner
from stage3_prediction.classification_head import ClassificationHead
from stage3_prediction.regression_head import RegressionHead


class ContextGuidedDetector(nn.Module):
    """
    上下文引导检测器
    
    完整架构：
    1. CLIP文本编码 + 全局上下文提取
    2. 查询初始化 + 上下文门控 + 文本调节
    3. 分类预测 + 边界框回归
    """
    
    def __init__(self, config):
        super().__init__()
        
        self.config = config
        
        # Stage1: 编码器
        self.text_encoder = CLIPTextEncoder(
            model_name=config.clip_model_name,
            pretrained_path=config.clip_checkpoint
        )
        
        self.context_extractor = GlobalContextExtractor(
            input_dim=config.d_clip,
            output_dim=config.d_model
        )
        
        # Stage2: 解码器
        self.query_initializer = QueryInitializer(
            num_queries=config.num_queries,
            d_model=config.d_model
        )
        
        self.context_gating = ContextGating(
            d_model=config.d_model,
            gating_type=config.context_gating_type
        )
        
        self.text_conditioner = TextConditioner(
            d_model=config.d_model,
            num_heads=config.num_heads
        )
        
        # Stage3: 预测头
        self.classification_head = ClassificationHead(
            d_model=config.d_model,
            num_classes=20  # DIOR
        )
        
        self.regression_head = RegressionHead(
            d_model=config.d_model
        )
    
    def forward(self, images, text_queries):
        """
        前向传播
        
        参数:
            images: (B, 3, H, W)
            text_queries: List of text queries
        
        返回:
            outputs: {
                'pred_logits': (B, M, num_classes),
                'pred_boxes': (B, M, 4),
                'text_features': (num_classes, d_clip)
            }
        """
        batch_size = images.size(0)
        
        # 1. 文本编码
        text_features = self.text_encoder(text_queries)  # (N, d_clip)
        
        # 2. 全局上下文提取 (需要图像编码器 - 待实现)
        # global_context = self.context_extractor(image_features)
        
        # 3. 查询初始化
        queries = self.query_initializer(batch_size)  # (B, M, d_model)
        
        # 4. 上下文门控 (需要视觉特征 - 待实现)
        # queries_gated = self.context_gating(queries, global_context)
        
        # 5. 文本调节
        # queries_conditioned = self.text_conditioner(queries_gated, text_features)
        
        # 6. 预测
        # pred_logits = self.classification_head(queries_conditioned, text_features)
        # pred_boxes = self.regression_head(queries_conditioned)
        
        # 临时返回（待完整实现）
        outputs = {
            'pred_logits': torch.zeros(batch_size, self.config.num_queries, 20),
            'pred_boxes': torch.zeros(batch_size, self.config.num_queries, 4),
            'text_features': text_features
        }
        
        return outputs


# 注意：这是一个框架，需要补充：
# 1. 图像编码器（提取视觉特征）
# 2. 特征融合的具体实现
# 3. 多层Transformer解码器（如果需要）
# 4. 完整的前向传播逻辑
```

---

## 🔧 组装步骤

### 步骤1: 补充图像编码器

需要创建 `stage1_encoder/clip_image_encoder.py`:

```python
class CLIPImageEncoder(nn.Module):
    """CLIP图像编码器"""
    
    def __init__(self, model_name='RN50', pretrained_path=None):
        super().__init__()
        # 加载RemoteCLIP图像编码器
        self.model, _, _ = open_clip.create_model_and_transforms(model_name)
        if pretrained_path:
            ckpt = torch.load(pretrained_path)
            self.model.load_state_dict(ckpt)
    
    def forward(self, images):
        return self.model.encode_image(images)
```

### 步骤2: 完善前向传播

连接所有模块：

```python
def forward(self, images, text_queries):
    # 1. 图像编码
    image_features = self.image_encoder(images)  # (B, d_clip)
    
    # 2. 文本编码
    text_features = self.text_encoder(text_queries)  # (N, d_clip)
    
    # 3. 全局上下文
    global_context = self.context_extractor(image_features)  # (B, d_model)
    
    # 4. 查询初始化
    queries = self.query_initializer(batch_size)  # (B, M, d_model)
    
    # 5. 上下文门控
    queries_gated = self.context_gating(queries, global_context)
    
    # 6. 文本调节
    queries_conditioned = self.text_conditioner(queries_gated, text_features)
    
    # 7. 预测
    pred_logits = self.classification_head(queries_conditioned, text_features)
    pred_boxes = self.regression_head(queries_conditioned)
    
    return {
        'pred_logits': pred_logits,
        'pred_boxes': pred_boxes
    }
```

### 步骤3: 创建训练脚本

参考 `experiment3/train.py` 的结构

### 步骤4: 测试

```bash
# 测试模型创建
python -c "from models.context_guided_detector import ContextGuidedDetector"

# 测试前向传播
python models/context_guided_detector.py
```

---

## 📚 参考资源

### Experiment3 参考

可以参考 Experiment3 的以下文件：

1. **完整模型**: `experiment3/models/ova_detr.py`
   - 模块组装方式
   - 前向传播逻辑
   - 特征维度匹配

2. **训练脚本**: `experiment3/train.py`
   - 训练循环
   - 优化器配置
   - 检查点保存

3. **评估脚本**: `experiment3/evaluate.py`
   - 评估流程
   - mAP计算
   - 结果保存

---

## ✅ 组装后的完整系统

完成后你将拥有：

```
experiment2/
├── stage1_encoder/
│   ├── clip_image_encoder.py    ✅ (需创建)
│   ├── clip_text_encoder.py     ✅
│   └── global_context_extractor.py  ✅
├── stage2_decoder/
│   ├── query_initializer.py     ✅
│   ├── context_gating.py        ✅
│   └── text_conditioner.py      ✅
├── stage3_prediction/
│   ├── classification_head.py   ✅
│   └── regression_head.py       ✅
├── stage4_supervision/
│   ├── box_loss.py              ✅
│   ├── global_contrast_loss.py  ✅
│   └── matcher.py               ✅
├── models/
│   └── context_guided_detector.py  ✅ (需完善)
├── utils/
│   ├── dataloader.py            ✅
│   ├── evaluation.py            ✅
│   └── box_utils.py             ✅
├── train.py                     ✅ (框架已有)
└── evaluate.py                  ✅ (框架已有)
```

---

## 🎯 下一步

1. 创建 `stage1_encoder/clip_image_encoder.py`
2. 完善 `models/context_guided_detector.py` 的前向传播
3. 测试模型创建和前向传播
4. 运行训练脚本
5. 评估性能

---

**创建时间**: 2025-10-24  
**状态**: 框架完成，待模型组装  
**预计工作量**: 2-3小时


