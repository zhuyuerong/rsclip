# Experiment1 坐标格式统一说明

## 📐 坐标格式标准

### 1. 边界框格式定义

**XYXY格式** (左上角 + 右下角):
```python
box = [x1, y1, x2, y2]
# x1, y1: 左上角坐标
# x2, y2: 右下角坐标
# 约束: x2 > x1, y2 > y1
```

**XYWH格式** (左上角 + 宽高):
```python
box = [x, y, w, h]
# x, y: 左上角坐标
# w: 宽度
# h: 高度
```

**CXCYWH格式** (中心点 + 宽高):
```python
box = [cx, cy, w, h]
# cx, cy: 中心点坐标
# w: 宽度
# h: 高度
```

### 2. 坐标系统

**像素坐标** (Pixel Coordinates):
```python
# 范围: [0, image_width] × [0, image_height]
# 单位: 像素
# 用于: 可视化、IoU计算
```

**归一化坐标** (Normalized Coordinates):
```python
# 范围: [0, 1] × [0, 1]
# 单位: 相对比例
# 用于: 模型输出、数据增强
```

---

## 📋 Experiment1 中的坐标格式

### Stage1: 提议生成

**输入**:
- 图像: PIL Image 或 numpy array
- 坐标: 像素坐标

**输出**:
- 候选框: `(N, 4)` **XYXY格式，像素坐标**

```python
# proposal_generator.py
proposals = [
    [x1, y1, x2, y2],  # XYXY像素坐标
    ...
]
```

### Stage2: 目标检测

**输入**:
- 候选框: **XYXY格式，像素坐标**
- 图像: PIL Image

**输出**:
- 检测框: **XYXY格式，像素坐标**
- 分数: float [0, 1]
- 类别: int

```python
# target_detector.py
detections = {
    'boxes': [[x1, y1, x2, y2], ...],  # XYXY像素坐标
    'scores': [0.95, 0.87, ...],
    'labels': [1, 5, ...]
}
```

### 边界框细化

**输入/输出**:
- **XYXY格式，像素坐标**

```python
# bbox_refiner.py
refined_box = refine_bbox(
    box,        # [x1, y1, x2, y2] 像素坐标
    image,
    context
)
# 返回: [x1, y1, x2, y2] 像素坐标
```

---

## 🔧 坐标转换函数

### XYXY ↔ XYWH

```python
def xyxy_to_xywh(box):
    """
    XYXY -> XYWH
    
    参数:
        box: [x1, y1, x2, y2]
    返回:
        box: [x, y, w, h]
    """
    x1, y1, x2, y2 = box
    x = x1
    y = y1
    w = x2 - x1
    h = y2 - y1
    return [x, y, w, h]


def xywh_to_xyxy(box):
    """
    XYWH -> XYXY
    
    参数:
        box: [x, y, w, h]
    返回:
        box: [x1, y1, x2, y2]
    """
    x, y, w, h = box
    x1 = x
    y1 = y
    x2 = x + w
    y2 = y + h
    return [x1, y1, x2, y2]
```

### XYXY ↔ CXCYWH

```python
def xyxy_to_cxcywh(box):
    """
    XYXY -> CXCYWH
    
    参数:
        box: [x1, y1, x2, y2]
    返回:
        box: [cx, cy, w, h]
    """
    x1, y1, x2, y2 = box
    cx = (x1 + x2) / 2
    cy = (y1 + y2) / 2
    w = x2 - x1
    h = y2 - y1
    return [cx, cy, w, h]


def cxcywh_to_xyxy(box):
    """
    CXCYWH -> XYXY
    
    参数:
        box: [cx, cy, w, h]
    返回:
        box: [x1, y1, x2, y2]
    """
    cx, cy, w, h = box
    x1 = cx - w / 2
    y1 = cy - h / 2
    x2 = cx + w / 2
    y2 = cy + h / 2
    return [x1, y1, x2, y2]
```

### 归一化 ↔ 像素

```python
def normalize_boxes(boxes, image_size):
    """
    像素坐标 -> 归一化坐标
    
    参数:
        boxes: (N, 4) 像素坐标
        image_size: (height, width)
    返回:
        boxes: (N, 4) 归一化坐标 [0, 1]
    """
    h, w = image_size
    boxes = boxes.copy()
    boxes[:, [0, 2]] /= w  # x坐标
    boxes[:, [1, 3]] /= h  # y坐标
    return boxes


def denormalize_boxes(boxes, image_size):
    """
    归一化坐标 -> 像素坐标
    
    参数:
        boxes: (N, 4) 归一化坐标
        image_size: (height, width)
    返回:
        boxes: (N, 4) 像素坐标
    """
    h, w = image_size
    boxes = boxes.copy()
    boxes[:, [0, 2]] *= w  # x坐标
    boxes[:, [1, 3]] *= h  # y坐标
    return boxes
```

---

## ✅ 使用规范

### 1. 函数参数标注

**强制要求**：在所有涉及边界框的函数中明确标注格式

```python
def my_function(boxes):
    """
    参数:
        boxes: (N, 4) [x1, y1, x2, y2] - XYXY格式，像素坐标
               或 (N, 4) [cx, cy, w, h] - CXCYWH格式，归一化坐标
    """
    pass
```

### 2. 变量命名规范

```python
# 推荐的变量命名：
boxes_xyxy          # XYXY格式
boxes_xywh          # XYWH格式
boxes_cxcywh        # CXCYWH格式
boxes_norm          # 归一化坐标
boxes_pixel         # 像素坐标
```

### 3. 坐标转换检查

```python
# 转换前检查
assert (boxes[:, 2] > boxes[:, 0]).all(), "x2 must > x1"
assert (boxes[:, 3] > boxes[:, 1]).all(), "y2 must > y1"

# 归一化检查
assert (boxes >= 0).all() and (boxes <= 1).all(), "Must be in [0, 1]"
```

---

## 🔍 常见问题

### Q1: IoU计算使用什么格式？

**A**: 始终使用 **XYXY格式，像素坐标**

```python
# 正确
iou = compute_iou(box1_xyxy_pixel, box2_xyxy_pixel)

# 错误
iou = compute_iou(box1_cxcywh, box2_cxcywh)  # 需要先转换
```

### Q2: 数据增强时如何处理坐标？

**A**: 保持格式不变，相应调整数值

```python
# 水平翻转 (XYXY格式)
x1_new = image_width - x2
x2_new = image_width - x1

# 缩放 (任何格式)
boxes *= scale_factor
```

### Q3: 如何在不同模块间传递边界框？

**A**: 统一使用 **XYXY格式，像素坐标**，在模块内部转换

```python
# 模块A输出
output_boxes = boxes_xyxy_pixel

# 模块B输入
def module_B(boxes_xyxy_pixel):
    # 内部转换为需要的格式
    boxes_cxcywh = xyxy_to_cxcywh(boxes_xyxy_pixel)
    ...
```

---

## 📝 检查清单

开发时检查：
- [ ] 函数参数是否标注了坐标格式？
- [ ] 变量命名是否清晰表明格式？
- [ ] IoU计算是否使用XYXY格式？
- [ ] 坐标转换是否正确？
- [ ] 归一化/反归一化是否正确处理了宽高？

---

**创建时间**: 2025-10-24  
**适用范围**: Experiment1 所有模块  
**标准**: 统一使用XYXY格式像素坐标作为主要格式

