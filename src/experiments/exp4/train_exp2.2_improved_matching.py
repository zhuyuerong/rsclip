...
def main():
    parser = argparse.ArgumentParser(description='训练改进匹配的SurgeryCAM模型')
    ...
    print(f"创建简化的SurgeryCAM模型（改进匹配）...")
    ...
    optimizer = torch.optim.AdamW([
        {'params': cam_params, 'lr': config.get('cam_generator_lr', 1e-5)},
        {'params': box_params, 'lr': config.get('learning_rate', 1e-4)}
    ], weight_decay=config.get('weight_decay', 0.01))
    
    print(f"优化器参数:")
    print(f"  CAM生成器学习率: {config.get('cam_generator_lr', 1e-5)}")
    print(f"  BoxHead学习率: {config.get('learning_rate', 1e-4)}")
    
    # 学习率调度器
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=config.get('num_epochs', 30), eta_min=1e-6
    )
    ...
    # 创建checkpoint目录
    checkpoint_dir = Path(config.get('checkpoint_dir', 'checkpoints/exp2.2'))
    checkpoint_dir.mkdir(parents=True, exist_ok=True)
    
    # 训练日志
    log_file = checkpoint_dir / f'training_exp2.2_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
    
    # 训练循环
    print(f"\n开始训练（实验2.2：改进匹配）...")
    print(f"总epoch数: {config.get('num_epochs', 30)}")
    print(f"训练样本数: {len(train_loader.dataset)}")
    print(f"日志文件: {log_file}")
    
    for epoch in range(start_epoch, config.get('num_epochs', 30)):
        ...
        # 记录日志
        log_msg = (f"Epoch [{epoch+1}/{config.get('num_epochs', 30)}] "
                  f"Loss: {avg_loss:.4f} | "
                  f"L1: {avg_l1:.4f} | "
                  f"GIoU: {avg_giou:.4f} | "
                  f"CAM: {avg_cam:.4f} | "
                  f"MatchIoU: {avg_match_iou:.4f} | "
                  f"PeakMatches: {match_stats['peak_matches']} | "
        ...
        # 保存最新checkpoint
        torch.save(checkpoint, checkpoint_dir / 'latest_exp2.2_model.pth')
        
        # 保存最佳checkpoint
        if avg_loss < best_loss:
            best_loss = avg_loss
            checkpoint['best_loss'] = best_loss
            torch.save(checkpoint, checkpoint_dir / 'best_exp2.2_model.pth')
