...
def train_epoch(model, dataloader, criterion, optimizer, device, epoch, config):
    ...
    # 使用tqdm但不显示详细进度，只显示epoch进度
    pbar = tqdm(dataloader, desc=f"Epoch [{epoch+1}/{config['num_epochs']}]", 
                leave=False, ncols=80, mininterval=60.0, disable=True)  # disable=True 完全禁用进度条
    ...
    return avg_loss, avg_l1, avg_giou, avg_cam


def main():
    parser = argparse.ArgumentParser(description='训练简化的SurgeryCAM模型')
    ...
    model = create_simple_surgery_cam_detector(
        ...
        device=device,
        unfreeze_cam_last_layer=True  # 解冻CAM生成器的最后一层
    )
    ...
    optimizer = torch.optim.AdamW([
        {'params': cam_params, 'lr': config.get('cam_generator_lr', config.get('cam_lr', 1e-5))},  # CAM生成器使用更小的学习率
        {'params': box_params, 'lr': config.get('learning_rate', 1e-4)}
    ], weight_decay=config.get('weight_decay', 0.01))
    
    print(f"优化器参数:")
    print(f"  CAM生成器学习率: {config.get('cam_generator_lr', config.get('cam_lr', 1e-5))}")
    print(f"  BoxHead学习率: {config.get('learning_rate', 1e-4)}")
    
    # 学习率调度器
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=config.get('num_epochs', 100), eta_min=1e-6
    )
    ...
    # 训练日志
    log_file = checkpoint_dir / f'training_simple_surgery_cam_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
    ...
    for epoch in range(start_epoch, config.get('num_epochs', 100)):
        avg_loss, avg_l1, avg_giou, avg_cam = train_epoch(
        ...
        # 保存最新checkpoint
        torch.save(checkpoint, checkpoint_dir / 'latest_simple_model.pth')
        
        # 保存最佳checkpoint
        if avg_loss < best_loss:
            best_loss = avg_loss
            checkpoint['best_loss'] = best_loss
            torch.save(checkpoint, checkpoint_dir / 'best_simple_model.pth')
