...
def main():
    parser = argparse.ArgumentParser(description='训练组合最佳方案的SurgeryCAM模型')
    parser.add_argument('--config', type=str, default='configs/exp4.1_combined_best.yaml',
                       help='配置文件路径')
    ...
    print(f"创建增强的SurgeryCAM模型（组合最佳方案）...")
    ...
    # 创建checkpoint目录
    checkpoint_dir = Path(config.get('checkpoint_dir', 'checkpoints/exp4.1'))
    checkpoint_dir.mkdir(parents=True, exist_ok=True)
    
    # 训练日志
    log_file = checkpoint_dir / f'training_exp4.1_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
    ...
    for epoch in range(start_epoch, config.get('num_epochs', 100)):
        ...
        torch.save(checkpoint, checkpoint_dir / 'latest_exp4.1_model.pth')
        
        if avg_loss < best_loss:
            best_loss = avg_loss
            checkpoint['best_loss'] = best_loss
            torch.save(checkpoint, checkpoint_dir / 'best_exp4.1_model.pth')
