...
def train_epoch(model, dataloader, criterion, optimizer, device, epoch, config):
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        loss_peak_sum += loss_dict.get('loss_peak', 0.0)
        num_batches += 1
    ...
    avg_peak = loss_peak_sum / num_batches
    
    avg_match_iou = 0.0
    if match_stats_agg['avg_match_iou']:
        avg_match_iou = sum(match_stats_agg['avg_match_iou']) / len(match_stats_agg['avg_match_iou'])
    
    return avg_loss, avg_l1, avg_giou, avg_cam, avg_peak, match_stats_agg, avg_match_iou


def main():
    parser = argparse.ArgumentParser(description='训练可学习峰值检测的SurgeryCAM模型')
    parser.add_argument('--config', type=str, default='configs/learnable_peak_config.yaml',
                       help='配置文件路径')
    ...
    criterion = LearnableDetectionLoss(
        ...
        init_threshold=config.get('init_threshold', 0.3),
        match_iou_threshold=config.get('match_iou_threshold', 0.2),
        use_objectness=config.get('use_objectness', True)
    ).to(device)
    ...
    # 创建checkpoint目录
    checkpoint_dir = Path(config.get('checkpoint_dir', 'checkpoints/learnable_peak'))
    checkpoint_dir.mkdir(parents=True, exist_ok=True)
    
    # 训练日志
    log_file = checkpoint_dir / f'training_learnable_peak_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
    ...
    for epoch in range(start_epoch, config.get('num_epochs', 50)):
        avg_loss, avg_l1, avg_giou, avg_cam, avg_peak, match_stats, avg_match_iou = train_epoch(
        ...
        if hasattr(criterion.assigner.peak_detector.detectors[0], 'threshold_logit'):
            current_threshold = torch.sigmoid(criterion.assigner.peak_detector.detectors[0].threshold_logit).item()
        ...
        checkpoint = {
            ...
            'criterion_state_dict': criterion.state_dict(),  # 保存峰值检测器参数
        ...
        torch.save(checkpoint, checkpoint_dir / 'latest_learnable_peak_model.pth')
        
        if avg_loss < best_loss:
            best_loss = avg_loss
            checkpoint['best_loss'] = best_loss
            torch.save(checkpoint, checkpoint_dir / 'best_learnable_peak_model.pth')
            print(f"✅ 保存最佳模型 (loss: {best_loss:.4f}, threshold: {current_threshold:.4f})")
