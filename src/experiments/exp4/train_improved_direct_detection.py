...
def main():
    parser = argparse.ArgumentParser(description='训练改进的直接检测模型')
    parser.add_argument('--config', type=str, default='configs/improved_direct_detection_config.yaml',
                       help='配置文件路径')
    ...
    print(f"使用设备: {device}")
    print(f"✅ 使用改进的直接检测方法：移除CAM损失，专注于检测质量")
    print(f"   损失函数: L1 + GIoU + 置信度（无CAM损失）")
    ...
    # 创建checkpoint目录
    checkpoint_dir = Path(config.get('checkpoint_dir', 'checkpoints/improved_direct_detection'))
    checkpoint_dir.mkdir(parents=True, exist_ok=True)
    
    # 训练日志
    log_file = checkpoint_dir / f'training_improved_direct_detection_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
    ...
    for epoch in range(start_epoch, config.get('num_epochs', 50)):
        ...
        torch.save(checkpoint, checkpoint_dir / 'latest_improved_direct_detection_model.pth')
        
        if avg_loss < best_loss:
            best_loss = avg_loss
            checkpoint['best_loss'] = best_loss
            torch.save(checkpoint, checkpoint_dir / 'best_improved_direct_detection_model.pth')
