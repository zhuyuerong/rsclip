...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # Store original path attention weights (before dropout for consistency)
        self.attn_weights_ori = attn_ori.detach()
        ...
        # Store VV path attention weights (before dropout for consistency)
        self.attn_weights_vv = attn.detach()
...
class VisionTransformer(nn.Module):
    ...
    def encode_image_with_all_tokens(self, x: torch.Tensor):
        """
        Encode image and return all tokens (cls token + patch tokens) without final projection.
        Used for extracting patch features for CAM generation.
        
        Args:
            x: [B, 3, H, W] input image tensor
            
        Returns:
            [B, 1+N², D] tensor containing cls token and all patch tokens
        """
        ...
        x = x.permute(1, 0, 2)  # NLD -> LND
        x, x_ori = self.transformer(x)
        # Use cls token from original path, patches from VV path
        x[0, :, :] = x_ori[0, :, :]  # clip_surgery
        x = x.permute(1, 0, 2)  # LND -> NLD
