...
# ---------------------------------------------------------
def _build_diff_vit(default_args: dict, pretrained: bool = False, **kwargs) -> DifferentialVisionTransformer:
    """
    Internal helper to build DifferentialVisionTransformer variants with a
    consistent set of defaults. Ensures the classifier head is disabled and
    pre-layer normalization is enabled so that external CLIP checkpoints can
    be mapped cleanly.
    """
    merged = dict(default_args)
    merged.update(kwargs)
    merged.setdefault('num_classes', 0)
    merged.setdefault('pre_norm', True)
    model = DifferentialVisionTransformer(**merged)
    # Ensure the classification head is an identity when num_classes == 0.
    if hasattr(model, 'head') and not isinstance(model.head, nn.Identity):
        model.head = nn.Identity()
    if pretrained:
        # Hook for potential future pretrained weight loading.
        pass
    return model
...
def diff_vit_base_patch16_224(pretrained: bool = False, **kwargs) -> DifferentialVisionTransformer:
    ...
    return _build_diff_vit(model_args, pretrained=pretrained, **kwargs)


@register_model
def diff_vit_base_patch32_224(pretrained: bool = False, **kwargs) -> DifferentialVisionTransformer:
    """
    Differential ViT-Base (ViT-B/32) with Differential Attention.

    This variant matches the architecture used by RemoteCLIP ViT-B/32 so that
    its weights can be adapted onto the differential transformer.
    """
    model_args = dict(
        patch_size=32,
        embed_dim=768,
